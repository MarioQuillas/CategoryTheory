...
using GeneralThings.MoreMonads;
...
public static partial class TryExtensions
{
    ...
    public static TResult Finally<T, TResult>(
        this Try<T> source, Func<Try<T>, TResult> finally) => finally

    (source);

    public static void Finally<T>(
        this Try<T> source, Action<Try<T>> finally) => finally
    ...
    internal static string Factorial(string value)
    {
        ...
        Try<int> query = from nullableInt32 in Try(() =>
                string.IsNullOrEmpty(value) ? default(int?) : Convert.ToInt32(value))
            // Try<int32?>
            from result in TryStrictFactorial(nullableInt32)
            // Try<int>.
            from unit in Try(() => result.WriteLine())
            // Try<Unit>.
